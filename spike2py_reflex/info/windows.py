...
@dataclass
class WindowTypes:
    extract: Union[Window, None] = Window()
    plotting: Union[Window, None] = Window()
...
class GroupedWindows:
    ...
    def _compute_idx_and_x_axes(self):
        ...
        for emg_name, reflex_windows in self.ms.reflexes.items():
            ...
            for reflex_type_, reflex_ms in reflex_windows.items():
                ...
                idx1_single = find_nearest_time_index(self.x_axes.single, reflex_ms[0] * s2pr.CONVERT_MS_TO_S)
                idx2_single = find_nearest_time_index(self.x_axes.single, reflex_ms[1] * s2pr.CONVERT_MS_TO_S)

                idx1_double = find_nearest_time_index(self.x_axes.double, reflex_ms[0] * s2pr.CONVERT_MS_TO_S)
                idx2_double = find_nearest_time_index(self.x_axes.double, reflex_ms[1] * s2pr.CONVERT_MS_TO_S)
                idx3_double = find_nearest_time_index(self.x_axes.double, (reflex_ms[0] + self.double_isi) * s2pr.CONVERT_MS_TO_S)
                idx4_double = find_nearest_time_index(self.x_axes.double, (reflex_ms[1] + self.double_isi) * s2pr.CONVERT_MS_TO_S)

                idx1_train = find_nearest_time_index(self.x_axes.train_single_pulse, reflex_ms[0] * s2pr.CONVERT_MS_TO_S)
                idx2_train = find_nearest_time_index(self.x_axes.train_single_pulse, reflex_ms[1] * s2pr.CONVERT_MS_TO_S)
        ...
        # sd
        self.idx.sd = dict()
        idx1_single = find_nearest_time_index(self.x_axes.single, self.ms.sd[0] * s2pr.CONVERT_MS_TO_S)
        idx2_single = find_nearest_time_index(self.x_axes.single, self.ms.sd[1] * s2pr.CONVERT_MS_TO_S)
        self.idx.sd['single'] = [idx1_single, idx2_single]
        self.idx.sd['double'] = None
        if self.double_isi is not None:
            idx1_double = find_nearest_time_index(self.x_axes.double, self.ms.sd[0] * s2pr.CONVERT_MS_TO_S)
            idx2_double = find_nearest_time_index(self.x_axes.double, self.ms.sd[1] * s2pr.CONVERT_MS_TO_S)
    ...
    def _get_extract_idx_and_x_axis(self, win: list):
        ...
        lower_idx = find_nearest_time_index(self._x_axis, win[0] * s2pr.CONVERT_MS_TO_S)
        upper_idx = find_nearest_time_index(self._x_axis, win[1] * s2pr.CONVERT_MS_TO_S)
