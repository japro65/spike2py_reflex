...
class Info:
    ...
    def __init__(self, study_path, plot=False):
        ...
        self.channels = s2pr.Channels(reflex_info["channels"])
        self.stim_params = s2pr.StimParams(reflex_info["stim_params"])
        self.trials_sections = s2pr.TrialsSections(reflex_info["trials_sections"])
        self.study_name: str = study_info["name"]
        self.windows = s2pr.Windows(reflex_info["windows"])
    ...
    def _get_study_info(self):
        study_info = s2pp.utils.read_json(self.study_path / s2pr.STUDY_INFO_FILE)
        reflex_info = s2pp.utils.read_json(self.study_path / s2pr.REFLEX_FILE)
    ...
    def init_subject(self, sub_id):
        ...
        try:
            path = self.study_path / sub_id / s2pr.SUBJECT_REFLEX_FILE
