...
class Triggers:
    ...
    def _remove_khz(self):
        ...
        for i in range(1, len(self._raw)):
            ...
            if trig_isi > s2pr.KHZ_ISI_THRESHOLD:
    ...
    def _get_idx(self, info):
        if self.type in [s2pr.SINGLE, s2pr.TRAIN]:
            self.extract = self._get_idx_single_triggers()
        elif self.type == s2pr.DOUBLE:
    ...
    def _is_double(self, isi, info):
        double_isi = info.stim_params.double_isi * s2pr.CONVERT_MS_TO_S
